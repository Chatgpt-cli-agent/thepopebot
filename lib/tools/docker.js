...
function buildAgentAuthEnv(agent) {
  ...
  if (agent === 'claude-code') {
    ...
    const builtinKeyMap = { anthropic: 'ANTHROPIC_API_KEY', openai: 'OPENAI_API_KEY', google: 'GOOGLE_API_KEY' };
    ...
    if (authMode === 'oauth') {
      ...
      const key = getConfig('CODEX_API_KEY');
      if (key) env.push(`CODEX_API_KEY=${key}`);
    }
